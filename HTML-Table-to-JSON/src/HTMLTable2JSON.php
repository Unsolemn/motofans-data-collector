...
class HTMLTable2JSON {
	...
	public function tableToJSON($url, $firstColIsRowName = TRUE, $tableID = '', $ignoreCols = NULL, $headers = NULL, $firstRowIsData = FALSE, $onlyColumns = FALSE, $arrangeByRow = FALSE, $ignoreHidden = FALSE, $printJSON = TRUE, $testingTable = NULL, $tableNum) {
		$ignoring = FALSE;
		$excluding = FALSE;
		if (NULL != $onlyColumns){
		...
		$html = str_ireplace("<br />", '', $html);
		$html = str_ireplace("<br/>", '', $html);
                /* $html = $this->extractNthTable($html,$tableNum); */
                /* $html = $this->extractTableByClass($html,"zebra"); */
                /* return $html; */
        ...
        private function extractNthTable($string,$nth)
        {
          $start = $end = false;
          $startTag = "<table";
          $endTag = "<\/table";
          $start = $this->strposX($string,$startTag,$nth);
          $end = $this->strposX($string,$endTag,$nth) + strlen($endTag);
          if(!$start || !$end) {
            echo ("Tag mismatch or not enough tables\n");
            return false;
          }
          echo "\n Table start at $start, ends at $end\n";
          $arr = substr($string,$start,$end-$start);
          return $arr;
        }

        private function extractTableByClass($string,$class)
        {
          $start = $end = false;
          $startTag = "<table class=\"$class";
          $endTag = "<\/table";
          $startTagCount = 0;
          /* $start = $this->strposX($string,$startTag,$nth); */
          
          $start = stripos($html, $startTag);
          $end = stripos($html, '</table>', $start_pos) + strlen('</table>');
          $length = $end_pos - $start_pos;
          $table = substr($html, $start_pos, $length);
          
          $end = $this->strposX($string,$endTag,$nth) + strlen($endTag);
          if(!$start || !$end) die ("Tag mismatch or not enough tables");
          echo "\n Table start at $start, ends at $end\n";
          $arr = substr($string,$start,$end-$start);
          return $arr;
        }
